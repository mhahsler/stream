...
Since the \code{DSC_Kmeans} clusterer returns the number of data points assignment to each
macro-cluster, we use this to calculate population percentage for the least populated
cluster.

We use a \code{DSD_Gaussians} data stream generator i and generate 3 clusters, each with his
own probability. Distinct probabilities direct how many data points is going to be generated
