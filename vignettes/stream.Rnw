...
<<>>=
reset_stream(stream)
p <- get_points(stream, n = 10000, info = TRUE)
head(p)
@

Outlier marks can be retrieved from the \code{.outlier} information column. These outliers were generated at positions
<<>>=
out_marks <- p[['.outlier']]
which(out_marks)
@
...
matplot(Position, cRand, type = "l", lwd = 1)
