...
\pkg{stream} provides implementation for many data stream clustering algorithms. The algorithms can be found using the registry \code{DSC_registry}.
...
The field \code{name} contains the function name to create the clusterer. More information on the algorithm can be found using help with \code{?} and the function name. \code{DSC_Micro} and \code{DSC_Macro} indicate if the algorithm implements a stream clustering algorithm that creates micro-clusters or if it is used for reclustering.
