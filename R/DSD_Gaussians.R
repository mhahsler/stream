...
#' @param sigma A list of length \code{k} of covariance matrices.
...
#' @param separation Depends on the \code{separation_type} parameter. It means
...
#' @param outliers Determines the number of data points marked as outliers.
...
#' \pkg{stream}" vignette.
#' @param outlier_options Effective only when \code{outliers>0}. Comprises the
...
#' outliers.
#'
#' \item\code{outlier_virtual_variance} - (Default=1) A variance
...
#' }
#' @param verbose Printout of the cluster and outlier generation process.
#' @return Returns a \code{DSD_Gaussians} object (subclass of \code{DSD_R},
...
#'
#' \item{mu}{The
#' matrix of means of the dimensions in each cluster.}
#'
#' \item{sigma}{The
#' covariance matrix.}
#'
    ...
    if (missing(mu)) {
      ...
      while (mu_index <= k) {
        ...
        while (i < 1000) {
          ...
          if (separation_type == "Euclidean" &&
              separation > 0 && !any(dist(rbind(mu, centroid)) < separation))
            break
      ...
      if (outliers > 0) {
        ...
        while (outs_index <= outliers) {
          ...
          while (i < 1000) {
            ...
            if (separation_type == "Euclidean" &&
                separation > 0 && !any(dist(rbind(rbind(outs, out), mu)) < separation))
              break
